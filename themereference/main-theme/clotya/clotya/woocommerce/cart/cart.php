		...
		<table class="table shop_table shop_table_responsive cart woocommerce-cart-form__contents" cellspacing="0">
			<thead>
				<tr>
					...
					<th scope="col" class="product-name"><?php esc_html_e( 'Product', 'clotya' ); ?></th>
					<th scope="col" class="product-price"><?php esc_html_e( 'Price', 'clotya' ); ?></th>
					<th scope="col" class="product-quantity"><?php esc_html_e( 'Quantity', 'clotya' ); ?></th>
					<th scope="col" class="product-subtotal"><?php esc_html_e( 'Subtotal', 'clotya' ); ?></th>
				</tr>
			</thead>
			<tbody>
				...
				foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
					...
					if ( $_product instanceof WC_Product && $_product->exists() && $cart_item['quantity'] > 0 && $visible ) {
						...
						<tr class="woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
							...
							<td class="product-remove">
								<?php
									echo apply_filters( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
										...
										sprintf(
											'<a role="button" href="%s" class="remove" aria-label="%s" data-product_id="%s" data-product_sku="%s"><i class="klbth-icon-cancel"></i></a>',
							...
							<td scope="row" role="rowheader" class="product-name" data-title="<?php esc_attr_e( 'Product', 'clotya' ); ?>">
							<?php
							if ( ! $product_permalink ) {
